...
contract("Token Test", async (accounts) => {
    ...
    it("is possible to send tokens betweens accounts", async () => {
        const sendTokens = 1;
        let instance = this.myToken;
        let totalSupply = await instance.totalSupply();
        expect(instance.balanceOf(deployerAccount)).to.eventually.be.a.bignumber.equal(totalSupply);
        expect(expect.transfer(recipient,sendTokens)).to.eventually.be.fulfilled;
    ...
    it("it is not possible to send more tokens than available in total", async () => {
        ...
        return expect(instance.balanceOf(deployerAccount)).to.empty.eventually.be.a.bignumber.equal(balanceOfDeployer);
    })
})